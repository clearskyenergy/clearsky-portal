/* ══════════════════════════════════════════════════════════════════════════
   OMEGA TERRAIN + ELECTRICAL PACK  ·  v2.2
   ──────────────────────────────────────────────────────────────────────────
   Closes the gap between the terrain OMEGA already samples and the
   flat-ground layout engine that never reads it.

   INSTALL: paste inside a script element immediately before the closing
   body tag, AFTER the application's own code. It wraps computeGroundLayout,
   so loading it before the fix pack assigns computeGroundLayoutV2 would
   wrap nothing.

   (Written without literal tag characters on purpose: a stray closing
   script tag anywhere in this file — even inside a comment — would end
   the block early and truncate the pack. The splice tool refuses to
   write when the tag count shifts, which is how this very line got
   ...
   v2.0  grid TIN replaces IDW sampling (3.6x accuracy, 28x speed);
         tracker axis fix; capacity iteration; cross-section tool
   v2.1  terrain-aware backtracking; per-pile local relief probe ring;
         swept-edge clearance at stow rotation
   v2.2  constraint colouring on the canvas; results persisted onto the
         shape so they survive a save and a reload
...
(function () {
  ...
  function checkCollisions(tables, presetKey, pileRes, proj, field, pxPerFt, opts) {
    opts = opts || {};
    var R = rackTuned(presetKey);
    if (R.pileMode === 'none' || !pileRes) return null;

    var clear = num(R.minGroundClearFt, 1.5);
    var strikes = [], breaks = [], buried = [], relief = [], swept = [];

    /* ---- per-pile local relief  (pack v2.1) -------------------------
       The plane and line fits average the ground under a table, so a
       boulder or a rut between sample points disappears into the
       residual. Ring eight probes around each solved pile at a small
       radius and compare against what the fit assumed. This is the check
       that a 0.1 m "checking radius" in commercial tools is doing. */
    var ringR = num(opts.checkRadiusFt, 0.33);          /* ~0.1 m */
    var reliefLim = num(opts.localReliefFt, 1.5);
    pileRes.piles.forEach(function (p) {
      var mn = Infinity, mx = -Infinity, seen = 0;
      for (var k = 0; k < 8; k++) {
        var ang = (Math.PI * 2 * k) / 8;
        var f = proj.toFt(p.px + ringR * pxPerFt * Math.cos(ang),
                          p.py + ringR * pxPerFt * Math.sin(ang));
        var z = field.zAt(f.x, f.y);
        if (z == null) continue;
        seen++;
        if (z < mn) mn = z;
      ...
      if (!seen) return;
      if (mx > p.topFt - 0.15) {
        buried.push({ tableIdx: p.tableIdx, seq: p.seq,
                      groundMaxFt: +mx.toFixed(2), topFt: +p.topFt.toFixed(2),
                      excessFt: +(mx - p.topFt).toFixed(2) });
      }
      if (mx - mn > reliefLim) {
        relief.push({ tableIdx: p.tableIdx, seq: p.seq, reliefFt: +(mx - mn).toFixed(2) });
      }
    });
    ...
    /* ---- swept-edge clearance at stow  (pack v2.1) ------------------
       A tracker does not collide at flat, it collides at stow. At the
       rotation limit the leading edge drops (chord/2)*sin(rot) below the
       tube and swings (chord/2)*cos(rot) to the side — on cross-sloped
       ground the downhill edge loses exactly the clearance the tube fit
       gained. So sweep both edges along the tube rather than sampling
       the centre once, and report the WORST point per table per side:
       one failing table produces two rows, not two hundred. */
    if (isTrackerPreset(presetKey)) {
      var rot = num(opts.maxRotDeg, 60) * D2R;
      /* The tube is a straight line, so its elevation at a station has to
         be INTERPOLATED. An earlier version compared the table's lowest
         pile top against ground anywhere along the tube, which on a
         sloped tube pits the low end's steel against the high end's dirt
         and failed roughly 70% of tables on a site that is actually
         buildable. Keep the end piles and interpolate between them. */
      var topBy = {};
      pileRes.piles.forEach(function (p) {
        var e = topBy[p.tableIdx];
        if (!e) { topBy[p.tableIdx] = { r0: p.runFt, z0: p.topFt, r1: p.runFt, z1: p.topFt }; return; }
        if (p.runFt < e.r0) { e.r0 = p.runFt; e.z0 = p.topFt; }
        if (p.runFt > e.r1) { e.r1 = p.runFt; e.z1 = p.topFt; }
      });
      for (var si = 0; si < tables.length; si++) {
        var t = tables[si], tp = topBy[si];
        if (!tp) continue;
        var g = tableGeom(t);
        var wFt = t.w / pxPerFt, hFt = t.h / pxPerFt;
        var chord = Math.min(wFt, hFt), tube = Math.max(wFt, hFt);
        var alongX = (wFt >= hFt);              /* tube on local x? */
        var drop = (chord / 2) * Math.sin(rot);
        var lat = (chord / 2) * Math.cos(rot);
        var worstSide = { '-1': null, '1': null };
        var steps = Math.max(2, Math.min(24, Math.round(tube / 12)));
        for (var q = 0; q <= steps; q++) {
          var d = -tube / 2 + (tube * q) / steps;
          for (var sd = -1; sd <= 1; sd += 2) {
            var lx = alongX ? d : (lat * sd);
            var ly = alongX ? (lat * sd) : d;
            var px = g.cx + (lx * g.ex.x + ly * g.ey.x) * pxPerFt;
            var py = g.cy + (lx * g.ex.y + ly * g.ey.y) * pxPerFt;
            var f2 = proj.toFt(px, py);
            var gz = field.zAt(f2.x, f2.y);
            if (gz == null) continue;
            var span = tp.r1 - tp.r0;
            var frac = (span > 1e-6) ? ((d - tp.r0) / span) : 0;
            var tubeZ = tp.z0 + frac * (tp.z1 - tp.z0);
            var clr = (tubeZ - drop) - gz;
            var cur = worstSide[String(sd)];
            if (clr < clear && (!cur || clr < cur.clearanceFt)) {
              worstSide[String(sd)] = {
                tableIdx: si, side: sd < 0 ? 'A' : 'B',
                stationFt: +d.toFixed(1),
                clearanceFt: +clr.toFixed(2), requiredFt: clear
              };
            }
          }
        }
        if (worstSide['-1']) swept.push(worstSide['-1']);
        if (worstSide['1']) swept.push(worstSide['1']);
      }
    }

    var hitTables = {};
    buried.forEach(function (x) { hitTables[x.tableIdx] = 1; });
    relief.forEach(function (x) { hitTables[x.tableIdx] = 1; });
    swept.forEach(function (x) { hitTables[x.tableIdx] = 1; });
    strikes.forEach(function (x) { hitTables[x.idx] = 1; });

    return {
      strikes: strikes, strikeCount: strikes.length,
      buried: buried, buriedCount: buried.length,
      localRelief: relief, localReliefCount: relief.length,
      swept: swept, sweptCount: swept.length,
      nearLimit: breaks, nearLimitCount: breaks.length,
      tablesAffected: Object.keys(hitTables).length,
      requiredClearFt: clear,
      checkRadiusFt: ringR,
      maxRotDeg: num(opts.maxRotDeg, 60)
    };
  }
  ...
  function exportStringCSV(res) {
    ...
    if (res.backtrack && res.backtrack.ok) {
      var B = res.backtrack;
      rows.push([], ['# backtracking \u2014 chord ' + B.chordFt + ' ft, pitch ' + B.pitchFt
                     + ' ft, worst row dz ' + B.worstDzFt + ' ft']);
      rows.push(['sun_theta_deg', 'flat_angle_deg', 'morning_deg', 'morning_corr_deg',
                 'evening_deg', 'evening_corr_deg']);
      B.table.forEach(function (r) {
        rows.push([r.thetaDeg, r.flatDeg, r.morningDeg, r.morningCorrDeg,
                   r.eveningDeg, r.eveningCorrDeg]);
      });
    }
  ...
  /* ══════════════════════════════════════════════════════════════════
     21.  TERRAIN-AWARE BACKTRACKING                        (pack v2.1)
     ──────────────────────────────────────────────────────────────────
     A tracker backtracks in the morning and evening: it rotates AWAY
     from the sun so the row in front does not shade the row behind. The
     flat-ground formula assumes both rows sit at the same elevation. On
     rolling ground they do not, and the error is one-sided — it always
     costs you the edge hours, never gives them back.

     Geometry, in the plane normal to the tracker axis. The neighbouring
     row is offset by (pitch, dz) with the SUNWARD row at +pitch, and the
     sun ray in that plane makes angle theta from vertical. The
     perpendicular gap between the parallel rays through the two axes is
     the cross product:

         gap = pitch * cos(theta) - dz * sin(theta)

     so a HIGHER sunward neighbour CLOSES the gap and demands more
     backtracking. No shading while the projected collector fits:

         chord * cos(theta - beta) <= gap

     which solves directly for beta. An earlier version of this had the
     dz sign the other way and rotated uphill rows toward the sun; the
     unit tests in the field pipeline caught it, which is why the sign is
     spelled out here rather than left to inspection.

     dz comes from the SOLVED pile tops, not raw ground, so the clamped
     tube slopes are what the controller actually sees. East and west are
     resolved through the real longitude of each row centroid rather than
     assumed from canvas orientation, which matters because the layout
     can sit at any azimuth.
     ══════════════════════════════════════════════════════════════════ */

  function backtrackAngle(thetaDeg, pitchFt, chordFt, dzFt) {
    var th = Math.abs(num(thetaDeg, 0)) * D2R;
    var dz = num(dzFt, 0);
    var sgn = (thetaDeg < 0) ? -1 : 1;

    function solve(gapNum) {
      var rhs = gapNum / chordFt;
      if (rhs >= 1) return th;              /* cannot shade: true-track   */
      if (rhs <= 0) return 0;               /* shaded regardless: go flat */
      var b = th - Math.acos(rhs);
      return b < 0 ? 0 : b;
    }
    var beta = solve(pitchFt * Math.cos(th) - dz * Math.sin(th));
    var flat = solve(pitchFt * Math.cos(th));

    return {
      angleDeg: +(sgn * beta * R2D).toFixed(2),
      flatAngleDeg: +(sgn * flat * R2D).toFixed(2),
      correctionDeg: +(sgn * (beta - flat) * R2D).toFixed(2),
      fullyShaded: (pitchFt * Math.cos(th) - dz * Math.sin(th)) <= 0
    };
  }

  /* Row-to-row height offsets, east-positive, from solved pile tops. */
  function rowOffsets(res) {
    if (!res || !res.piles || !res.kept.length) return [];
    var P = ppf();
    var pitchPx = num(res.pitchFt, 0) * P;
    if (!(pitchPx > 0)) return [];

    /* Bucket tables into rows along the pitch axis of their own frame. */
    var a = res.kept[0].a || 0, c = Math.cos(a), s = Math.sin(a);
    var rows = {};
    var topBy = res.topByTable || {};
    res.kept.forEach(function (t, i) {
      var top = topBy[i];
      if (top == null) return;
      var cx = t.x + t.w / 2, cy = t.y + t.h / 2;
      var lv = (-cx * s + cy * c);              /* along the pitch axis, px */
      var k = Math.round(lv / pitchPx);
      var r = rows[k] || (rows[k] = { z: 0, n: 0, px: 0, py: 0 });
      r.z += top; r.n++; r.px += cx; r.py += cy;
    });

    var keys = Object.keys(rows).map(Number).sort(function (p, q) { return p - q; });
    var out = [];
    for (var i = 0; i + 1 < keys.length; i++) {
      if (keys[i + 1] - keys[i] !== 1) continue;      /* gap in the field */
      var A = rows[keys[i]], B = rows[keys[i + 1]];
      var zA = A.z / A.n, zB = B.z / B.n;

      /* Which of the pair is actually east? Ask the projector rather than
         assuming, because the array can sit at any azimuth. */
      var eastIsB = true;
      try {
        var llA = res.proj.toLatLng(A.px / A.n, A.py / A.n);
        var llB = res.proj.toLatLng(B.px / B.n, B.py / B.n);
        if (llA && llB) eastIsB = (llB.lng >= llA.lng);
      } catch (e) {}

      out.push({
        row: keys[i],
        dzEastFt: +((eastIsB ? (zB - zA) : (zA - zB))).toFixed(2)
      });
    }
    return out;
  }

  /* Whole-array backtracking report at a few representative sun angles. */
  function backtrackReport(res, opts) {
    opts = opts || {};
    res = res || LAST;
    if (!res || !res.ok) return { ok: false, why: 'Run the terrain analysis first.' };

    var R = rackTuned(res.presetKey);
    if (R.pileMode === 'none' || !isTrackerPreset(res.presetKey)) {
      return { ok: false, why: 'Backtracking applies to trackers. This array is ' + R.label + '.' };
    }

    var offs = rowOffsets(res);
    if (!offs.length) return { ok: false, why: 'Not enough adjacent rows to compare.' };

    var chord = Math.min(num(res.chordFt, 0) || Infinity,
                         num(LAST.kept[0].w, 0) / ppf(),
                         num(LAST.kept[0].h, 0) / ppf());
    if (!(chord > 0) || !isFinite(chord)) chord = 6;
    var pitch = num(res.pitchFt, chord / 0.33);

    var dzs = offs.map(function (o) { return o.dzEastFt; });
    var absSorted = dzs.map(Math.abs).sort(function (p, q) { return p - q; });
    var worst = dzs.reduce(function (a, b) { return Math.abs(b) > Math.abs(a) ? b : a; }, 0);
    var median = absSorted[Math.floor(absSorted.length / 2)];
    /* Drive the angle table from a REPRESENTATIVE row, not the worst one.
       A single 40% pair on an otherwise gentle site pins every angle at
       zero and reports an array that never tracks, which is false for all
       but that one pair. The worst pair is reported alongside, as a
       flag — it is a row to fix, not a controller setting. */
    var p90 = absSorted[Math.min(absSorted.length - 1, Math.floor(absSorted.length * 0.9))];
    var repDz = (worst < 0 ? -1 : 1) * median;

    /* Sample the tracking day: theta is the sun's angle from vertical in
       the rotation plane, so large theta is early or late. */
    var thetas = opts.thetas || [45, 60, 70, 80];
    var table = thetas.map(function (th) {
      var am = backtrackAngle(th, pitch, chord, repDz);      /* sun east */
      var pm = backtrackAngle(th, pitch, chord, -repDz);     /* sun west */
      return {
        thetaDeg: th,
        flatDeg: am.flatAngleDeg,
        morningDeg: am.angleDeg, morningCorrDeg: am.correctionDeg,
        eveningDeg: pm.angleDeg, eveningCorrDeg: pm.correctionDeg
      };
    });

    var maxCorr = table.reduce(function (m, r) {
      return Math.max(m, Math.abs(r.morningCorrDeg), Math.abs(r.eveningCorrDeg));
    }, 0);

    return {
      ok: true,
      rowPairs: offs.length,
      chordFt: +chord.toFixed(2),
      pitchFt: +pitch.toFixed(2),
      gcr: +(chord / pitch).toFixed(3),
      medianDzFt: +median.toFixed(2),
      p90DzFt: +p90.toFixed(2),
      worstDzFt: +worst.toFixed(2),
      basisDzFt: +repDz.toFixed(2),
      outlierRows: offs.filter(function (o) { return Math.abs(o.dzEastFt) > 4 * (median || 0.5); }).length,
      table: table,
      maxCorrectionDeg: +maxCorr.toFixed(2),
      offsets: offs
    };
  }
  ...
  /* ══════════════════════════════════════════════════════════════════
     22.  CONSTRAINT COLOURING                              (pack v2.2)
     ──────────────────────────────────────────────────────────────────
     Everything above this section produces numbers. A panel can tell you
     663 tables were rejected; it cannot tell you they are all in the
     north-east corner, which is the thing that decides whether you
     regrade a corner or move the fence.

     HOW IT SURVIVES A RE-RENDER. The base renderer rebuilds every rect
     from scratch, and it is re-run by settleRender, _geoReprojectAll,
     pushHist and the __omegaGround wrapper — double renders are routine.
     Painting the rects once would work until the first pan.

     So the status is written onto the TABLE OBJECTS (t._omegaPaint) and
     _derRenderAutoLayout is wrapped to apply it after the base renderer
     has drawn. Re-rendering ten times gives the same colours as
     rendering once, and because the tag rides on the table it also
     survives a project save.

     MODES. Each answers a different question:
       status     is this table buildable, and if not, why
       slope      how close to the limit is it, as a continuous ramp
       reveal     which piles need cut and which need fill — two
                  colours, two meanings, both actionable by a civil
                  engineer, per the build map
       clearance  where structure meets ground
       off        hand the array back to the base renderer untouched
     ══════════════════════════════════════════════════════════════════ */

  var PAINT_MODES = ['off', 'status', 'slope', 'reveal', 'clearance'];
  var _paintMode = 'status';

  var PAINT_COLORS = {
    ok:        { fill: 'rgba(47,181,74,.55)',  stroke: '#1F7A33', label: 'buildable' },
    along:     { fill: 'rgba(229,83,75,.62)',  stroke: '#A5342E', label: 'axis slope' },
    cross:     { fill: 'rgba(217,164,65,.62)', stroke: '#8A6520', label: 'cross slope' },
    warp:      { fill: 'rgba(176,131,240,.62)',stroke: '#6B44A8', label: 'twist' },
    noData:    { fill: 'rgba(110,118,129,.5)', stroke: '#484F58', label: 'no data' },
    cut:       { fill: 'rgba(229,83,75,.62)',  stroke: '#A5342E', label: 'needs cut' },
    fill:      { fill: 'rgba(88,166,255,.62)', stroke: '#2C6FBF', label: 'needs fill' },
    both:      { fill: 'rgba(176,131,240,.62)',stroke: '#6B44A8', label: 'cut and fill' },
    strike:    { fill: 'rgba(229,83,75,.7)',   stroke: '#A5342E', label: 'grade strike' },
    swept:     { fill: 'rgba(217,164,65,.66)', stroke: '#8A6520', label: 'swept edge' },
    relief:    { fill: 'rgba(176,131,240,.55)',stroke: '#6B44A8', label: 'local relief' },
    unknown:   { fill: 'rgba(37,99,235,.55)',  stroke: '#1D4ED8', label: 'not analysed' }
  };

  /* Continuous green -> amber -> red by fraction of the limit used. */
  function rampColor(frac) {
    var f = clamp(num(frac, 0), 0, 1.35);
    var r, g, b;
    if (f <= 0.5) {                       /* green -> amber */
      var u = f / 0.5;
      r = Math.round(47 + (217 - 47) * u);
      g = Math.round(181 + (164 - 181) * u);
      b = Math.round(74 + (65 - 74) * u);
    } else {                              /* amber -> red */
      var v = clamp((f - 0.5) / 0.5, 0, 1);
      r = Math.round(217 + (229 - 217) * v);
      g = Math.round(164 + (83 - 164) * v);
      b = Math.round(65 + (75 - 65) * v);
    }
    return { fill: 'rgba(' + r + ',' + g + ',' + b + ',.6)',
             stroke: 'rgb(' + Math.round(r * .65) + ',' + Math.round(g * .65) + ',' + Math.round(b * .65) + ')' };
  }

  /* Write paint tags onto the table objects of the analysed array. */
  function paintTables(res, mode) {
    res = res || LAST;
    mode = mode || _paintMode;
    if (!res || !res.ok) return 0;

    var sh = null;
    try {
      sh = (S.shapes || []).filter(function (x) { return x.id === res.shapeId; })[0];
    } catch (e) {}
    if (!sh || !sh.tables) return 0;

    /* Clear first: a table that was rejected last run and is buildable
       now must lose its old colour, and modes disagree about which
       tables they even mention. */
    sh.tables.forEach(function (t) { t._omegaPaint = null; });
    if (mode === 'off') { redraw(); return 0; }

    var R = rackTuned(res.presetKey);
    var idxOf = {};
    res.kept.forEach(function (t, i) { idxOf[i] = t; });

    /* status: reason-coded */
    if (mode === 'status') {
      res.kept.forEach(function (t) { t._omegaPaint = PAINT_COLORS.ok; });
      res.rejected.forEach(function (r) {
        var w = r.why || '';
        var key = w.indexOf('axis') === 0 ? 'along'
                : w.indexOf('cross') === 0 ? 'cross'
                : w.indexOf('twist') === 0 ? 'warp' : 'noData';
        r.t._omegaPaint = PAINT_COLORS[key];
      });
    }

    /* slope: continuous, as a fraction of whichever limit binds hardest */
    else if (mode === 'slope') {
      var limA = num(res.screen.limitAlongPct, R.slopeAlongPct);
      var limC = num(res.screen.limitCrossPct, R.slopeCrossPct);
      sh.tables.forEach(function (t) {
        var s = t._slope;
        if (!s) { t._omegaPaint = PAINT_COLORS.noData; return; }
        var fa = limA > 0 ? Math.abs(s.slopeAlongPct) / limA : 0;
        var fc = limC > 0 ? Math.abs(s.slopeCrossPct) / limC : 0;
        var c = rampColor(Math.max(fa, fc));
        t._omegaPaint = { fill: c.fill, stroke: c.stroke,
                          label: Math.max(fa, fc).toFixed(2) + '\u00d7 limit' };
      });
    }

    /* reveal: cut vs fill, the two-colours-two-meanings view */
    else if (mode === 'reveal') {
      if (!res.piles) return 0;
      var byTable = {};
      res.piles.piles.forEach(function (p) {
        var e = byTable[p.tableIdx] || (byTable[p.tableIdx] = { cut: 0, fill: 0 });
        if (p.status === 'cut') e.cut++;
        else if (p.status === 'fill') e.fill++;
      });
      res.kept.forEach(function (t, i) {
        var e = byTable[i];
        if (!e) { t._omegaPaint = PAINT_COLORS.ok; return; }
        t._omegaPaint = (e.cut && e.fill) ? PAINT_COLORS.both
                      : e.cut ? PAINT_COLORS.cut
                      : e.fill ? PAINT_COLORS.fill
                      : PAINT_COLORS.ok;
      });
    }
    ...
    /* clearance: where structure meets ground */
      ...
      var c = res.collisions;
      if (!c) return 0;
      res.kept.forEach(function (t) { t._omegaPaint = PAINT_COLORS.ok; });
      c.localRelief.forEach(function (x) {
        if (idxOf[x.tableIdx]) idxOf[x.tableIdx]._omegaPaint = PAINT_COLORS.relief;
      });
      c.swept.forEach(function (x) {
        if (idxOf[x.tableIdx]) idxOf[x.tableIdx]._omegaPaint = PAINT_COLORS.swept;
      });
      /* strikes and buried piles are hard stops, so they win the colour */
      c.strikes.forEach(function (x) {
        if (idxOf[x.idx]) idxOf[x.idx]._omegaPaint = PAINT_COLORS.strike;
      });
      c.buried.forEach(function (x) {
        if (idxOf[x.tableIdx]) idxOf[x.tableIdx]._omegaPaint = PAINT_COLORS.strike;
      });
    }

    var n = 0;
    sh.tables.forEach(function (t) { if (t._omegaPaint) n++; });
    _paintMode = mode;
    redraw();
    return n;
  }
  ...
  function redraw() {
    try { if (typeof settleRender === 'function') { settleRender(); return; } } catch (e) {}
    try { if (typeof renderAll === 'function') { renderAll(); return; } } catch (e) {}
    try { if (typeof render === 'function' && render.length === 0) render(); } catch (e) {}
  }

  /* Wrap the array renderer so paint is re-applied after every rebuild. */
  function installPainter() {
    var orig = window._derRenderAutoLayout;
    if (typeof orig !== 'function' || orig.__omegaPaint) return false;

    var wrapped = function (g, sh) {
      var r = orig.call(this, g, sh);
      try {
        if (!sh || !sh.tables || !sh.tables.length) return r;
        var painted = false;
        for (var i = 0; i < sh.tables.length; i++) {
          if (sh.tables[i]._omegaPaint) { painted = true; break; }
        }
        if (!painted) return r;

        /* The base renderer appends one rect.o2-tbl per table, in table
           order, so index alignment holds. Query inside THIS shape's
           group only — several arrays can share the canvas. */
        var rects = g.querySelectorAll ? g.querySelectorAll('rect.o2-tbl') : null;
        if (!rects || rects.length !== sh.tables.length) return r;
        for (var j = 0; j < rects.length; j++) {
          var p = sh.tables[j]._omegaPaint;
          if (!p) continue;
          rects[j].setAttribute('fill', p.fill);
          rects[j].setAttribute('stroke', p.stroke);
          rects[j].setAttribute('stroke-width', '0.5');
        }
      } catch (e) {}
      return r;
    };
    wrapped.__omegaPaint = true;
    window._derRenderAutoLayout = wrapped;
    return true;
  }

  /* Legend for whichever mode is showing. */
  function legendFor(mode, res) {
    var keys = mode === 'status'    ? ['ok', 'along', 'cross', 'warp', 'noData']
             : mode === 'reveal'    ? ['ok', 'cut', 'fill', 'both']
             : mode === 'clearance' ? ['ok', 'relief', 'swept', 'strike']
             : null;
    if (mode === 'slope') {
      var stops = [0, .25, .5, .75, 1], sw = '';
      stops.forEach(function (f) {
        sw += '<span style="display:inline-block;width:26px;height:9px;background:'
           + rampColor(f).fill + '"></span>';
      });
      return '<div class="note" style="margin-top:7px">' + sw
        + '<div style="display:flex;justify-content:space-between;width:130px;margin-top:2px">'
        + '<span>0</span><span>at limit</span></div>'
        + 'Fraction of the binding slope limit each table uses.</div>';
    }
    if (!keys) return '';
    var h = '<div class="note" style="margin-top:7px;line-height:1.9">';
    keys.forEach(function (k) {
      var c = PAINT_COLORS[k];
      h += '<span style="display:inline-block;width:9px;height:9px;border-radius:2px;'
        + 'background:' + c.fill + ';border:1px solid ' + c.stroke + ';margin-right:4px"></span>'
        + c.label + '&nbsp;&nbsp;';
    });
    return h + '</div>';
  }
  ...
  function analyze(opts) {
    ...
    /* 3. collisions */
    var coll = checkCollisions(screen.kept, presetKey, piles, proj, field, P, opts);
    ...
    LAST = {
      ...
      pitchFt: num(sh.pitchFt, 0),
      chordFt: Math.min(num(sh.tableWft, 0) || Infinity, num(sh.tableHft, 0) || Infinity),
      at: Date.now()
    };
    /* Backtracking reads the solved pile tops, so it runs last. */
    try { LAST.backtrack = backtrackReport(LAST, opts); }
    catch (e) { LAST.backtrack = { ok: false, why: (e && e.message) || 'failed' }; }

    /* Persist the headline result onto the SHAPE, not just this object.
       Until now everything lived in LAST and vanished on reload, so
       nothing downstream — BOM, permit sheets, the one-line — could
       read an analysis that had already been run. */
    try { persistToShape(sh, LAST); } catch (e) {}

    /* Colour the canvas. This is the point of the whole exercise: a
       count of rejected tables does not tell you they are all in one
       corner, and that is what decides whether you regrade or move the
       fence. */
    try { paintTables(LAST, _paintMode); } catch (e) {}
  ...
  /* ══════════════════════════════════════════════════════════════════
     23.  PERSISTENCE                                       (pack v2.2)
     ──────────────────────────────────────────────────────────────────
     The build map asks for solved geometry to be stored per table so
     downstream stages read it rather than recomputing. Piles are written
     onto the table in the shape the map specifies, and a compact summary
     onto the shape, so a saved project carries its analysis.

     Deliberately NOT stored: the projector and the field. Those are
     closures over a live terrain lattice and would not survive
     JSON.stringify — a saved project reloads the summary and the pile
     schedule, and re-running the analysis rebuilds the rest.
     ══════════════════════════════════════════════════════════════════ */

  function persistToShape(sh, res) {
    if (!sh || !res || !res.ok) return false;

    /* Per-table piles, in the field-pipeline shape:
       {station,x,y,groundZ,topZ,reveal,length,flag} */
    if (res.piles) {
      var byTable = {};
      res.piles.piles.forEach(function (p) {
        (byTable[p.tableIdx] || (byTable[p.tableIdx] = [])).push({
          station: +p.runFt.toFixed(2),
          x: +p.xFt.toFixed(2), y: +p.yFt.toFixed(2),
          groundZ: +p.groundFt.toFixed(2), topZ: +p.topFt.toFixed(2),
          reveal: +p.revealFt.toFixed(3), length: +p.lengthFt.toFixed(2),
          flag: p.status
        });
      });
      res.kept.forEach(function (t, i) { t.piles = byTable[i] || null; });
    }

    sh.terrainAnalysis = {
      at: res.at,
      packVersion: VERSION,
      sampler: res.field.kind,
      spacingFt: Math.round(res.field.spacingFt),
      projection: res.projMode,
      quality: res.projQuality,
      presetKey: res.presetKey,
      tablesTotal: res.kept.length + res.rejected.length,
      buildable: res.screen.keptCount,
      buildablePct: res.screen.buildablePct,
      kwFlat: res.kwFlat, kwReal: res.kwReal,
      reasons: res.screen.reasons,
      piles: res.piles ? {
        count: res.piles.count,
        revealMinFt: res.piles.revealMinFt,
        revealMaxFt: res.piles.revealMaxFt,
        outOfRange: res.piles.outOfRange,
        steelFt: res.piles.totalSteelFt,
        cutCy: res.piles.cutCy, fillCy: res.piles.fillCy
      } : null,
      clearance: res.collisions ? {
        tablesAffected: res.collisions.tablesAffected,
        strikes: res.collisions.strikeCount,
        buried: res.collisions.buriedCount,
        localRelief: res.collisions.localReliefCount,
        swept: res.collisions.sweptCount
      } : null,
      shading: res.shading && res.shading.ok ? {
        actualPitchFt: res.shading.actualPitchFt,
        requiredPitchFt: res.shading.requiredPitchWindowFt,
        maxGcr: res.shading.maxGcrWindow,
        clearHours: res.shading.clearHours,
        verdict: res.shading.verdict
      } : null,
      backtrack: res.backtrack && res.backtrack.ok ? {
        rowPairs: res.backtrack.rowPairs,
        medianDzFt: res.backtrack.medianDzFt,
        worstDzFt: res.backtrack.worstDzFt,
        maxCorrectionDeg: res.backtrack.maxCorrectionDeg
      } : null,
      dc: res.strings ? {
        modules: res.strings.totalModules,
        strings: res.strings.stringCount,
        combiners: res.strings.combinerCount,
        blocks: res.strings.blockCount,
        cableFt: res.strings.totalDcFt
      } : null
    };
  ...
  var CSS = ''
    ...
    + '#omega-terr .modes{display:flex;gap:3px;margin:2px 0 0}'
    + '#omega-terr .modes button{flex:1;padding:5px 2px;background:#252526;border:1px solid #3C3C3C;'
    + 'color:#8FA3B8;border-radius:4px;font-size:8.5px;cursor:pointer;font-family:inherit;font-weight:600}'
    + '#omega-terr .modes button:hover{color:#E2EEF9}'
    + '#omega-terr .modes button.on{background:rgba(47,181,74,.16);border-color:rgba(47,181,74,.45);color:#2FB54A}';
  ...
  function render(res) {
    ...
    h += '<h4>Colour the canvas by</h4><div class="modes">';
    PAINT_MODES.forEach(function (m) {
      h += '<button data-paint="' + m + '"' + (m === _paintMode ? ' class="on"' : '') + '>'
         + (m === 'off' ? 'none' : m) + '</button>';
    });
    h += '</div>' + legendFor(_paintMode, res);
    ...
    if (c) {
      h += '<h4>Clearance</h4>'
        + row('Tables affected', c.tablesAffected, c.tablesAffected ? 'a' : 'g')
        + row('Grade strikes', c.strikeCount, c.strikeCount ? 'x' : 'g')
        + row('Piles buried', c.buriedCount, c.buriedCount ? 'x' : 'g')
        + row('Local relief flags', c.localReliefCount, c.localReliefCount ? 'a' : 'g')
        + row('Swept edge @ ' + c.maxRotDeg + '\u00b0 stow', c.sweptCount, c.sweptCount ? 'x' : 'g')
        + row('Required clearance', c.requiredClearFt + ' ft')
        + row('Rows near the limit', c.nearLimitCount, c.nearLimitCount ? 'a' : 'g');
      if (c.buriedCount || c.sweptCount) {
        h += '<div class="note">Buried piles and swept-edge hits are hard stops \u2014 the '
           + 'structure meets ground. Local relief is a warning: the fit averaged over '
           + 'something the ' + c.checkRadiusFt + ' ft probe ring found.</div>';
      }
    }

    var bt = res.backtrack;
    if (bt && bt.ok) {
      h += '<h4>Backtracking</h4>'
        + row('Row pairs', bt.rowPairs)
        + row('Chord \u00b7 pitch', bt.chordFt + ' \u00b7 ' + bt.pitchFt + ' ft (GCR ' + bt.gcr + ')')
        + row('Row \u0394z med / p90 / worst',
              bt.medianDzFt + ' / ' + bt.p90DzFt + ' / ' + bt.worstDzFt + ' ft')
        + row('Angles computed on', bt.basisDzFt + ' ft (median row)')
        + row('Max terrain correction', bt.maxCorrectionDeg + '\u00b0',
              Math.abs(bt.maxCorrectionDeg) > 3 ? 'a' : 'g');
      h += '<div class="note" style="margin-top:5px">'
         + '<table style="width:100%;border-collapse:collapse;font-size:9px">'
         + '<tr style="color:#8FA3B8"><th style="text-align:right">sun \u03b8</th>'
         + '<th style="text-align:right">flat</th><th style="text-align:right">AM</th>'
         + '<th style="text-align:right">PM</th></tr>';
      bt.table.forEach(function (r) {
        h += '<tr><td style="text-align:right">' + r.thetaDeg + '\u00b0</td>'
           + '<td style="text-align:right">' + r.flatDeg + '\u00b0</td>'
           + '<td style="text-align:right">' + r.morningDeg + '\u00b0</td>'
           + '<td style="text-align:right">' + r.eveningDeg + '\u00b0</td></tr>';
      });
      h += '</table>Flat-ground backtracking under-rotates on sloped rows and the loss '
         + 'is one-sided \u2014 it always costs edge hours, never returns them.';
      if (bt.outlierRows) {
        h += '<br><br>' + bt.outlierRows + ' row pair(s) sit far outside the rest '
           + '(worst ' + bt.worstDzFt + ' ft over ' + bt.pitchFt + ' ft). Those are rows to '
           + 're-grade or drop, not a reason to detune the whole array \u2014 the angles above '
           + 'are computed on the median row.';
      }
      h += '</div>';
    } else if (bt && bt.why && isTrackerPreset(res.presetKey)) {
      h += '<h4>Backtracking</h4><div class="note">' + esc(bt.why) + '</div>';
    }
  ...
  function wire(b) {
    ...
    Array.prototype.forEach.call(b.querySelectorAll('[data-paint]'), function (btn) {
      btn.onclick = function () {
        var m = btn.getAttribute('data-paint');
        var n = paintTables(LAST, m);
        say(m === 'off' ? 'Array colouring off.' : (n + ' tables coloured by ' + m + '.'));
        render(LAST);
      };
    });
  ...
  window.OmegaTerrain = {
    ...
    set paintMode(m) { if (PAINT_MODES.indexOf(m) >= 0) paintTables(LAST, m); },
    capacity: capacitySearch,
    section: sectionArm,
    backtrack: backtrackReport,
    backtrackAngle: backtrackAngle,
