...
/* ══════════════════════════════════════════════════════════════════════════
   PER-CLIENT SAVE / RESUME  (added for Phase 2)
   ----------------------------------------------------------------------------
   Saves every input to Firestore at  toolData/{orgId}/tools/investment  so a
   client can pause a long analysis and pick it up later. Scoped per tenant via
   the ?org= param (see OMEGATools.orgFromUrl), so SPATCO's saved work and
   NextNRG's never mix. ES5 only. Degrades silently if Firebase isn't present.
   ══════════════════════════════════════════════════════════════════════════ */
var IA_TOOL_KEY = 'investment';
var IA_db = null, IA_org = null, IA_ready = false, IA_saveTimer = null;

/* Collect every input on the page into a plain object. */
function iaCollect(){
  var out = {}, nodes = document.querySelectorAll('input[id], select[id], textarea[id]');
  for (var i=0;i<nodes.length;i++){
    var e = nodes[i];
    if (!e.id) continue;
    // only capture the analysis inputs (a_ / i_ / iv_), skip UI-only controls
    if (!/^(a_|i_|iv_)/.test(e.id)) continue;
    out[e.id] = (e.type==='checkbox') ? !!e.checked : e.value;
  }
  return out;
}

/* Restore a saved object into the fields (mirrors loadClaremontPreset). */
function iaRestore(data){
  if (!data) return;
  var k, e;
  for (k in data){ if (!data.hasOwnProperty(k)) continue;
    e = document.getElementById(k); if (!e) continue;
    if (e.type==='checkbox') e.checked = !!data[k]; else e.value = data[k];
  }
  if (typeof syncModuleInputs==='function') syncModuleInputs();
  if (typeof seedValueStack==='function') { /* keep util-derived fields consistent */ }
}

/* Debounced save — called on input changes and after Run. */
function iaSave(){
  if (!IA_ready || !IA_db || !IA_org) return;
  if (IA_saveTimer) clearTimeout(IA_saveTimer);
  IA_saveTimer = setTimeout(function(){
    OMEGATools.saveToolData(IA_db, firebase, IA_org, IA_TOOL_KEY, iaCollect())
      .then(function(){ iaSetSaved('Saved'); })
      ['catch'](function(){ iaSetSaved('Save failed'); });
  }, 900);
}

function iaSetSaved(msg){
  var s = document.getElementById('iv-save-status');
  if (s){ s.textContent = msg; if (msg==='Saved'){ s.style.opacity='1'; setTimeout(function(){ s.style.opacity='0.55'; }, 1200); } }
}

/* Wire up save/resume once Firebase auth resolves. */
function iaInitPersistence(){
  if (typeof firebase==='undefined' || !window.CLEARSKY_CONFIG){ return; }
  try { firebase.initializeApp(window.CLEARSKY_CONFIG.firebase); }
  catch(e){ if(!/already exists/.test(e.message)) return; }
  IA_db = firebase.firestore();
  firebase.auth().onAuthStateChanged(function(user){
    IA_org = OMEGATools.orgFromUrl(user ? (user.email||'') : '');
    if (!IA_org){ return; }               // no org context → run as a plain calculator
    IA_ready = true;
    // Offer to resume a saved analysis.
    OMEGATools.loadToolData(IA_db, IA_org, IA_TOOL_KEY).then(function(saved){
      if (saved && saved.data && Object.keys(saved.data).length){
        iaShowResume(saved.data, (saved.updatedAt && saved.updatedAt.toDate) ? saved.updatedAt.toDate() : null);
      }
    });
    // Auto-save on any input change.
    document.addEventListener('change', function(ev){
      if (ev.target && ev.target.id && /^(a_|i_|iv_)/.test(ev.target.id)) iaSave();
    });
...
/* Small non-blocking resume banner. */
function iaShowResume(data, when){
  var host = document.getElementById('run-status');
  if (!host){ return; }
  var wrap = document.createElement('div');
  wrap.style.cssText = 'margin:10px 0;padding:10px 14px;border:1px solid #C9A84C;background:rgba(201,168,76,.10);border-radius:8px;font-size:13px;display:flex;gap:12px;align-items:center;flex-wrap:wrap';
  var label = when ? ('You have a saved analysis from ' + when.toLocaleString() + '.') : 'You have a saved analysis.';
  wrap.innerHTML = '<span>'+label+'</span>';
  var resume = document.createElement('button');
  resume.textContent = 'Resume'; resume.className='btn btn-navy'; resume.style.cssText='padding:6px 14px;font-size:12px;cursor:pointer';
  resume.onclick = function(){ iaRestore(data); wrap.parentNode.removeChild(wrap); if(typeof ivRun==='function') ivRun(); };
  var fresh = document.createElement('button');
  fresh.textContent = 'Start fresh'; fresh.className='btn btn-ghost'; fresh.style.cssText='padding:6px 14px;font-size:12px;cursor:pointer';
  fresh.onclick = function(){ wrap.parentNode.removeChild(wrap); };
  wrap.appendChild(resume); wrap.appendChild(fresh);
  host.parentNode.insertBefore(wrap, host);
}

/* Boot persistence after the page's own boot(). */
if (document.readyState==='loading') document.addEventListener('DOMContentLoaded', iaInitPersistence);
else iaInitPersistence();
